...
Write-Host "====================================="
Write-Host "Cortex Backend Startup"
Write-Host "====================================="
...
# Check if cortex-frontend is installed
$frontendInstalled = $false
$frontendPath = $null

# Check common installation paths for cortex-frontend
$possiblePaths = @(
    "$env:LOCALAPPDATA\Programs\cortex-frontend\cortex-frontend.exe",
    "$env:LOCALAPPDATA\Programs\cortex\cortex-frontend.exe",
    "$env:ProgramFiles\cortex-frontend\cortex-frontend.exe",
    "${env:ProgramFiles(x86)}\cortex-frontend\cortex-frontend.exe",
    "$env:APPDATA\cortex-frontend\cortex-frontend.exe"
)
...
# Also check if cortex is in the registry (properly installed)
try {
    ...
    if ($regPath) {
        ...
        if (-not $frontendPath -and $regPath.InstallLocation) {
            ...
            $possibleExeNames = @("cortex-frontend.exe", "cortex.exe")
...
if ($frontendInstalled) {
    Write-Host "Cortex frontend application is installed."
    if ($frontendPath) {
        Write-Host "Location: $frontendPath"
        
        # Ask if user wants to launch the frontend
        $launchFrontend = Read-Host "Do you want to launch the Cortex frontend application? (Y/N)"
        if ($launchFrontend -eq 'Y' -or $launchFrontend -eq 'y') {
            Write-Host "Launching Cortex frontend..."
            Start-Process -FilePath $frontendPath
            Write-Host "Cortex frontend launched successfully."
        }
    }
} else {
    Write-Host "Cortex frontend application is not installed."
    Write-Host ""
    $installFrontend = Read-Host "Would you like to download and install the Cortex frontend application? (Y/N)"
        ...
        try {
            ...
            if ($setupAsset) {
                ...
                if ($installerProcess.ExitCode -eq 0) {
                    ...
                    # Check if cortex is now installed and launch it
