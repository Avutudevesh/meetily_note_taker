...
function Find-ExistingDatabases {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\.cortex\meeting_minutes.db",
        "$env:USERPROFILE\Documents\cortex\meeting_minutes.db",
    ...
    if ($env:HOMEBREW_PREFIX) {
        $commonPaths += "$env:HOMEBREW_PREFIX/Cellar/cortex-backend/*/backend/meeting_minutes.db"
    }
