...
$AppProjectName = "cortex-backend"
$AppContainerName = "cortex-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "cortex-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server cortex-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server cortex-backend
...
function Invoke-LogsCommand {
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "cortex-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, cortex-backend"
        ...
        $logArgs += @("whisper-server", "cortex-backend")
    }
